...
class TBorderWidget(QWidget):
    def __init__(self, *args, pos=(0, 0), length=30, **kwargs):
        super(TBorderWidget, self).__init__(*args, **kwargs)
        self.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowMinimizeButtonHint)
        self.setAttribute(Qt.WA_TranslucentBackground)
        self.length = length
        self.lengths = dict((i, length * i / 6) for i in range(1, 13))
    ...
    def pos(self):
        x, y = super(TBorderWidget, self).pos().toTuple()
        return int(x - self.center[0]), int(y - self.center[1])

    def move(self, *arg__1):
        x, y = arg__1
        super(TBorderWidget, self).move(x + self.center[0], y + self.center[1])
        pass
